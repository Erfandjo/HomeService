...
{
    public class CommentAppService(ICommentService _commentService , IExpertService _expertService) : ICommentAppService
    {
        public async Task<Result> Add(CommentCreateDto comment , CancellationToken cancellation)
        {
           ...
           await _expertService.ChangeScore(comment.ExpertId, comment.Star, cancellation);
